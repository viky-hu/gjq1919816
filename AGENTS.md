...
The repo also tracks a Next.js monorepo under `final-main/` (the BFF/UI that calls these endpoints). The old `newfront/` directory was replaced by this path; keep the federation contract below in sync with `final-main/`.
...
3. **Vendored fork** — `LightRAG/` — the upstream graph-RAG engine (entity/relation extraction, KG storage, query modes). It has its own `LightRAG/AGENTS.md`, `LightRAG/AGENTS.md`, and `LightRAG/.clinerules/01-basic.md`; read those for fork internals instead of re-deriving them.
