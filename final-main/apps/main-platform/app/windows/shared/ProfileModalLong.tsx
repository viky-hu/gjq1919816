...
export function ProfileModalLong({ onClose }: ProfileModalLongProps) {
  ...
  const handleApplyCenterNode = useCallback(() => {
    ...
    // 提交管理员申请，等待审批
    void adminSubmitRequest(username, "申请成为中心节点", "用户申请将本机节点提权为中心节点").then(() => {
      setCenterNodePending(true);
      setApplyState("success");
    }).catch(() => {
      setApplyState("failed");
    });
  }, [applyState, isSelfCenterNode, judgeModelConfigured, setIsSelfCenterNode, setCenterNodePending, username]);
  ...
  useEffect(() => {
    ...
    setFadingLocation(null);
    // 如果正在等待中心节点审批，不要重置 applyState
    if (!centerNodePending) {
      setApplyState("idle");
    }
    setNameApplyState("idle");
    setM1AvatarHint("");
    ...
    resetAvatarDraft();
  }, [resetAvatarDraft, savedNodeLocation, centerNodePending]);
      ...
      <div className="global-top-nav__profile-content global-top-nav__profile-content--long">
        ...
        <section className="global-top-nav__section">
          ...
          {isSelfCenterNode ? (
            ...
            <button
              type="button"
              className="global-top-nav__long-apply-btn is-loading"
              disabled
            >
              申请中，等待管理员审核…
            </button>
          ...
          <span
            className={`global-top-nav__save-hint${applyState === "failed" || (applyState === "success" && isSelfCenterNode === false) || centerNodePending ? " global-top-nav__save-hint--warning" : ""}`}
            aria-live="polite"
          >
            {applyMessage}
          </span>
