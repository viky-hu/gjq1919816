...
function buildPlateNodes(
  ...
  apiNodes: Array<{ id: string; label: string; nodeType: string; isHome: boolean }> = [],
...
function resolveCursorTone(node: NodeConfig): CursorTone {
  // isRed flag takes priority (set when user matches this node as their own center node)
...
export function D3SandboxThreeMvp(props: D3SandboxProps) {
  const { username, isSelfCenterNode: runtimeIsSelfCenterNode, isAdmin } = useAppRuntime();
  ...
  const [nodeMetrics, setNodeMetrics] = useState<Record<string, { activity: number; connections: number; totalRecords: number; todayUpdates: number; callCount: number; uptime: number }>>({});
  const [apiNodes, setApiNodes] = useState<Array<{ id: string; label: string; nodeType: string; isHome: boolean }>>([]);
  ...
  const selfCenterEnabled = (props.isSelfCenterNode ?? false) || isAdmin;
  ...
  const plateNodes = useMemo(
    () => buildPlateNodes(props.selfNodeName ?? DEFAULT_SELF_NODE_NAME, selfCenterEnabled, props.selfNodePlacement ?? null, apiNodes),
    [props.selfNodeName, selfCenterEnabled, props.selfNodePlacement, apiNodes, isAdmin],
  );
  ...
  useEffect(() => {
    ...
    fetch(`${miaRagUrl}/api/macro/nodes`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    })
      .then((r) => {
        if (!r.ok) console.warn("[D3] /api/macro/nodes HTTP", r.status);
  ...
  useEffect(() => {
    ...
    // Find plate containing the self node (isRed) for center node users
    let selfNodePlateId: PlateId | null = null;
    if (!selfPlacementPlateId) {
      for (const pid of PLATE_IDS) {
        if (plateNodes[pid]?.some((n) => n.isRed)) {
          selfNodePlateId = pid;
          break;
        }
      }
    }
    const defaultPlateId = selfPlacementPlateId ?? selfNodePlateId ?? DEFAULT_ACTIVE_PLATE_ID;
    ...
    const autoSelectNodeId = (() => {
      // Find the self node (isRed) or fallback to "node-center-red"
      const targetPlate = defaultPlateId;
      if (targetPlate) {
        const selfNode = plateNodes[targetPlate]?.find((n) => n.isRed);
        if (selfNode) return selfNode.id;
      }
      return "node-center-red";
    })();
    const nodeTimer = window.setTimeout(() => {
      if (visible) {
        setSelectedNodeId(autoSelectNodeId);
        onNodeSelect(autoSelectNodeId);
      }
                ...
                {isSelfOrdinaryNode ? (
                  <>
                    <div className="d3-info-metric d3-info-metric--self-normal">
                      <span className="d3-info-metric-val d3-info-metric-val--self-normal">{nodeMetrics[activeNode.id]?.activity ?? 0}</span>
                      <span className="d3-info-metric-lbl d3-info-metric-lbl--self-normal">活跃度%</span>
                    </div>
                    <div className="d3-info-metric d3-info-metric--self-normal">
                      <span className="d3-info-metric-val d3-info-metric-val--self-normal">{nodeMetrics[activeNode.id]?.connections ?? 0}</span>
                      <span className="d3-info-metric-lbl d3-info-metric-lbl--self-normal">连接数</span>
                    </div>
                    <div className="d3-info-metric d3-info-metric--self-normal">
                      <span className="d3-info-metric-val d3-info-metric-val--self-normal">{(nodeMetrics[activeNode.id]?.totalRecords ?? 0).toLocaleString()}</span>
                  ...
                  <>
                    ...
                    <div className="d3-info-metric">
                      <span className="d3-info-metric-val">{nodeMetrics[activeNode.id]?.todayUpdates ?? 0}</span>
                      <span className="d3-info-metric-lbl">今日更新</span>
                    </div>
                    <div className="d3-info-metric">
                      <span className="d3-info-metric-val">{(nodeMetrics[activeNode.id]?.callCount ?? 0).toLocaleString()}</span>
                      <span className="d3-info-metric-lbl">调用次数</span>
                    </div>
                    <div className="d3-info-metric">
                      <span className="d3-info-metric-val">{nodeMetrics[activeNode.id]?.uptime ?? 99.8}%</span>
                  ...
                  <>
                    <div className="d3-info-metric">
                      <span className="d3-info-metric-val">{nodeMetrics[activeNode.id]?.activity ?? 0}</span>
                      <span className="d3-info-metric-lbl">活跃度%</span>
                    </div>
                    <div className="d3-info-metric">
                      <span className="d3-info-metric-val">{nodeMetrics[activeNode.id]?.connections ?? 0}</span>
                      <span className="d3-info-metric-lbl">连接数</span>
                    </div>
                    <div className="d3-info-metric">
                      <span className="d3-info-metric-val">{(nodeMetrics[activeNode.id]?.totalRecords ?? 0).toLocaleString()}</span>
                      <span className="d3-info-metric-lbl">记录总数</span>
                    </div>
                  </>
                ) : null}
              </div>
