...
async function fetchContributions(period: Period): Promise<ContributionData | null> {
  ...
  try {
    ...
    const res = await fetch(`${miaRagUrl}/api/macro/node-contributions?period=${apiPeriod}`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
      signal: AbortSignal.timeout(5000),
    });
    if (!res.ok) return null;
    const data = await res.json() as { contributions?: ContributionData };
    return data.contributions && Object.keys(data.contributions).length > 0 ? data.contributions : null;
  } catch {
    return null;
  }
}
...
export function D4Visualization({ visible, activeSectorId, selectedNodeId, selfNodeName }: D4VisualizationProps) {
  ...
  const [apiContributions, setApiContributions] = useState<ContributionData | null>(null);
  ...
  useEffect(() => {
    if (!visible) return;
    let cancelled = false;
    setApiContributions(null);
    setApiLoaded(false);
    fetchContributions(period).then((data) => {
      if (!cancelled) {
        setApiContributions(data);
        setApiLoaded(true);
      }
    }).catch(() => {
  ...
  }, [period, resolvedNodeSeriesId, displayNodeId, apiContributions]);
  ...
  const isEmpty = apiLoaded && data.length === 0;
  ...
  return (
    <div ref={rootRef} className="d4viz-root" aria-label={`${nodeLabel} 数据库价值监测`}>
      ...
      {isEmpty ? (
        <div className="d4viz-empty">
          <p>暂无监测数据</p>
          <p className="d4viz-empty-hint">上传文献后将自动生成数据</p>
        </div>
      ) : (
