...
async function fetchWordCloud(nodeId?: string | null): Promise<WordCloudDatum[] | null> {
  ...
  try {
    ...
    const url = nodeId
      ? `${miaRagUrl}/api/macro/word-cloud?node_id=${encodeURIComponent(nodeId)}`
      : `${miaRagUrl}/api/macro/word-cloud`;
    const res = await fetch(url, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
      signal: AbortSignal.timeout(5000),
    });
    if (!res.ok) {
      console.warn("[WordCloud] HTTP", res.status);
      return null;
    }
    const data = await res.json() as { words?: Array<{ text: string; weight: number }>; keywords?: Array<{ word: string; count: number }> };
    // Backend returns { words: [{ text, weight }] }, also support legacy { keywords: [{ word, count }] }
    if (data.words && data.words.length > 0) {
      return data.words.map((w) => ({ text: w.text, weight: w.weight }));
    }
    ...
    console.warn("[WordCloud] fetch failed:", e);
    return null;
  }
}
...
function buildWordCloudLayout(source: WordCloudDatum[], width: number, height: number): LayoutWord[] {
  ...
  // Aesthetic font size: use text hash to assign a curated size tier,
  // not driven by word frequency/weight. Produces a pleasing mix of
  // large hero words, medium support words, and small detail words.
  const aestheticTier = (text: string, index: number) => {
    let hash = 0;
    for (let i = 0; i < text.length; i++) {
      hash = ((hash << 5) - hash + text.charCodeAt(i)) | 0;
    }
    const seed = Math.abs(hash + index * 7) % 100;
    if (seed < 15) return 1.0;        // ~15% hero (largest)
    if (seed < 40) return 0.72;       // ~25% large
    if (seed < 70) return 0.5;        // ~30% medium
    if (seed < 88) return 0.33;       // ~18% small
    return 0.18;                      // ~12% tiny (detail)
  };
  ...
  sorted.forEach((word, index) => {
    const ratio = aestheticTier(word.text, index);
...
export function D5WordCloud({ visible, activeSectorId, selectedNodeId, selfNodeName }: D5WordCloudProps) {
  ...
  // Fetch real word cloud data only when a specific node is selected (not plate click)
  useEffect(() => {
    if (!visible || !selectedNodeId || selectedNodeId === "node-center-red" || selectedNodeId === "node-current") {
      setLiveWords(null);
      setApiLoaded(false);
      return;
    }
    setLiveWords(null);
    setApiLoaded(false);
    fetchWordCloud(selectedNodeId).then((words) => {
      setApiLoaded(true);
  ...
  // Only use real API data — empty means no files uploaded yet
  const sourceWords = liveWords ?? [];
  ...
  const isEmpty = !selectedNodeId || (apiLoaded && sourceWords.length === 0);
  ...
  return (
    <div ref={rootRef} className="d5cloud-root" aria-label="语义地点词汇云图">
      ...
      {isEmpty ? (
        <div className="d5cloud-empty">
          <p>{selectedNodeId ? "当前节点暂无文献数据" : "请选择一个节点"}</p>
          <p className="d5cloud-empty-hint">{selectedNodeId ? "上传文献后将自动生成词云" : ""}</p>
        </div>
      ) : (
