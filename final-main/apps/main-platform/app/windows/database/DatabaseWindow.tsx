...
export function DatabaseWindow({ onBack, onNavigateToMain, onOpenMacro }: DatabaseWindowProps) {
  ...
  const [renameTargetCluster, setRenameTargetCluster] = useState<Cluster | null>(null);
  ...
  useLayoutEffect(() => {
    ...
    // CSS-first: set initial hidden state, then animate TO visible
  ...
  useEffect(() => {
    ...
    if (renameTargetCluster) {
      const id = setTimeout(() => renameInputRef.current?.focus(), 50);
      return () => clearTimeout(id);
    }
  }, [isModalOpen, renameTargetCluster]);
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape" && renameTargetCluster && !isRenamingCluster) setRenameTargetCluster(null);
    };
    window.addEventListener("keydown", handleKey);
    return () => window.removeEventListener("keydown", handleKey);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isModalOpen, deleteTargetCluster, isDeletingCluster, renameTargetCluster, isRenamingCluster]);
  ...
  const openRenameModal = useCallback((cluster: Cluster) => {
  ...
  const handleConfirmRename = useCallback(async () => {
    ...
    if (!name) { setRenameError("聚类名称不能为空"); return; }
    if (name.length > 50) { setRenameError("名称不能超过 50 个字符"); return; }

    setIsRenamingCluster(true);
    try {
      const res = await fetch(`/api/database/clusters/${renameTargetCluster.id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          ...authHeaders,
      ...
      if (!res.ok) {
        const data = await res.json() as { error?: string };
        setRenameError(data.error ?? "重命名失败，请重试");
        return;
      }
      setRenameTargetCluster(null);
      if (selectedCluster?.id === renameTargetCluster.id) {
        setSelectedCluster((prev) => prev ? { ...prev, name } : prev);
      }
      await fetchData();
    } catch {
  ...
  }, [authHeaders, renameTargetCluster, renameValue, fetchData, selectedCluster?.id, username]);
    ...
    <div ref={rootRef} className="db-window">
              ...
              clusters.map((cluster) => (
                ...
                >
                  ...
                  <div className="db-cluster-main">
                    ...
                    <button
                      className="db-cluster-rename-btn"
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                        openRenameModal(cluster);
                      }}
                      onKeyDown={(e) => e.stopPropagation()}
                      aria-label={`重命名聚类：${cluster.name}`}
                    >
                      重命名
                    </button>
                  </div>
                  ...
                  <div className="db-cluster-side">
                    ...
                    <div className="db-cluster-actions">
                      <button
      ...
      {renameTargetCluster && (
        <div
          ...
          role="presentation"
          onClick={(e) => {
            if (e.target === e.currentTarget && !isRenamingCluster) {
              closeRenameModal();
            }
          }}
        >
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="db-rename-cluster-title"
          >
            <div className="db-modal-header">
              <p className="db-modal-eyebrow">DATABASE · RENAME</p>
              <h3 id="db-rename-cluster-title" className="db-modal-title">
                重命名聚类
              </h3>
            </div>

            <div className="db-modal-body">
              <label className="db-modal-label" htmlFor="db-rename-cluster-input">
                新名称
              </label>
              <input
                id="db-rename-cluster-input"
                ref={renameInputRef}
                className="db-modal-input"
                type="text"
                value={renameValue}
                onChange={(e) => { setRenameValue(e.target.value); setRenameError(""); }}
                onKeyDown={(e) => e.key === "Enter" && !isRenamingCluster && handleConfirmRename()}
                placeholder="输入新名称…"
                maxLength={50}
                aria-describedby={renameError ? "db-rename-error" : undefined}
                aria-invalid={!!renameError}
                autoComplete="off"
              />
              {renameError && (
                <p id="db-rename-error" className="db-modal-error" role="alert">
                  {renameError}
                </p>
              )}
            </div>
            ...
            <div className="db-modal-footer">
              ...
              <button
                ...
                type="button"
                disabled={isRenamingCluster}
                aria-busy={isRenamingCluster}
              >
                {isRenamingCluster ? "保存中…" : "确认重命名"}
              </button>
            </div>
          </div>
        </div>
