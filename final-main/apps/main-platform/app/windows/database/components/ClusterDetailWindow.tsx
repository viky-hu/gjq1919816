...
async function readTextWithEncoding(file: File): Promise<string> {
  ...
  // Check for replacement character (U+FFFD) which indicates encoding mismatch
  if (!utf8.includes("�")) {
    return utf8;
  }

  // Try GBK/GB2312 for Chinese text files
  try {
    const gbk = new TextDecoder("gbk", { fatal: true }).decode(bytes);
    return gbk;
  } catch {
    // GBK failed, fall back to UTF-8 with replacement
    return utf8;
  }
}
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const uploadTasks = selected.map(async (file) => {
      ...
      if (shouldPersistAsText(file)) {
        textContent = await readTextWithEncoding(file);
      } else {
