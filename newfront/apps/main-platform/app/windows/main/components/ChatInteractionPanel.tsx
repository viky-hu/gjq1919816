...
function stripMarkdown(text: string): string {
  return text
    ...
    .replace(/^\|.*\|$/gm, "")           // table rows → remove
    .replace(/^[-=]{3,}$/gm, "")         // horizontal rules
    // Remove references/bibliography section
    .replace(/(?:^|\n)(?:references?|参考文献|引用来源|来源|sources?|bibliography)[\s\S]*$/i, "")
    // Remove inline reference markers like [1], [2], [1][2]
    .replace(/\[\d+\]/g, "")
    ...
    typingTimerRef.current = window.setTimeout(() => {
      ...
      botReplyTimerRef.current = window.setTimeout(() => {
        ...
        void (async () => {
          ...
          const replyText = stripMarkdown(resolvedReply.answer);
