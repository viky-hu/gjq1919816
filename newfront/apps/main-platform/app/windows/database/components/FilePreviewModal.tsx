...
export function FilePreviewModal({ file, name, addedAt, highlightPhrases = [], onClose }: FilePreviewModalProps) {
  ...
  // Entrance animation — useLayoutEffect + CSS-first to prevent FOUC
  useLayoutEffect(() => {
    ...
    gsap.set(modal, { autoAlpha: 0, y: 24, scale: 0.96 });
    gsap.to(overlay, { autoAlpha: 1, duration: 0.22, ease: "power2.out" });
    gsap.to(modal,
