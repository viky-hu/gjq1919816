...
import { shouldUseOfflineBackendFallback } from "../offline-demo";

const DEFAULT_CENTRAL_TIMEOUT_MS = 120_000;
...
function resolveCentralBaseUrl(): string {
  const raw = process.env.FEDERATION_CENTRAL_BASE_URL;
  if (shouldUseOfflineBackendFallback(raw)) return "";
  return normalizeFederationServiceUrl(raw, "FEDERATION_CENTRAL_BASE_URL");
}
...
export async function askCentral(question: string, requestId: string = randomUUID()): Promise<AskCentralResult> {
  ...
  if (!baseUrl) {
    return {
      requestId,
      status: "partial",
      answer: `演示模式已收到中心聚合问题：“${parsed.data.question}”。当前未配置中心服务，已返回本地占位结果。`,
      details: [
        {
          node: "本机节点",
          status: "ok",
