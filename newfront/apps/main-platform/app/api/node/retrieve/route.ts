...
import { retrieveNodeAnswer } from "@/app/lib/server/node-data";
...
const RetrieveSchema = z.object({
  ...
  mode: z.enum(["local", "global", "hybrid", "mix", "naive"]).default("mix"),
});
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (shouldUseOfflineBackendFallback(miaRagUrl)) {
      const localResult = await retrieveNodeAnswer({
    ...
    const miaTimeout = setTimeout(() => miaController.abort(), 120_000);
    ...
    try {
      const miaRes = await fetch(`${miaRagUrl}/api/query/internal`, {
        ...
        body: JSON.stringify({ question: payload.question, account: payload.account, mode: payload.mode }),
      ...
      // If details is empty, try to build from top-level evidence
      if (details.length === 0 && Array.isArray(miaData.evidence)) {
        for (const item of miaData.evidence.slice(0, 5)) {
          const e = item as { source?: string; content?: string; relevance?: number; score?: number };
          if (e.content) {
            details.push({
              clusterId: e.source ?? "mia-rag",
              fileId: e.source ?? "mia-rag",
              fileName: e.source ?? "MiA-RAG",
              score: typeof e.relevance === "number" ? e.relevance : (typeof e.score === "number" ? e.score : 0),
              snippet: String(e.content).slice(0, 500),
            });
          }
        }
      }

      const answer = miaData.answer ?? "";
      if (!answer) {
        return NextResponse.json(
          {
            error: {
              code: "MIA_RAG_EMPTY_ANSWER",
              message: "MiA-RAG 后端未返回有效回答，请检查服务状态和知识库是否已加载",
              requestId,
            },
          },
          { status: 502 },
