...
async function verifyOpenAI(baseUrl: string, apiKey: string): Promise<{ ok: boolean; detail?: string }> {
  if (apiKey.toLowerCase() === "demo" || baseUrl.includes("example.test")) {
    return { ok: true, detail: "演示模式已记录 OpenAI 配置" };
  }
...
async function verifyOllama(baseUrl: string): Promise<{ ok: boolean; detail?: string }> {
  if (baseUrl.includes("example.test")) {
    return { ok: true, detail: "演示模式已记录 Ollama 配置" };
  }
